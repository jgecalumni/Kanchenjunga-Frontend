...
const Page = ({ params }: { params: Promise<{ id: string }> }) => {
	...
	const handleSubmit = async (values: any) => {
		...
		try {
			...
			let finalvalues: any = { ...values, id: userData.id };
			const res = await createReview({ id, data: finalvalues }).unwrap();
			if (res.success) {
	...
	useEffect(() => {
		if (isError) {
			toast.error((error as any).data.message || "Failed to create review");
		}
	}, [isError, error]);
					...
					<Card className="border border-gray-200 bg-gradient-to-r from-blue-50 to-purple-50">
						<CardContent className="p-4">
							<div className="flex gap-4">
								<div className="relative w-48 rounded-lg overflow-hidden flex-shrink-0">
									<Image
					...
					{({ values, setFieldValue, errors, touched }) => (
						<Form className="space-y-6">
							...
							{token ? (
								...
								<div className="text-red-500 font-semibold text-[14px] text-center">
